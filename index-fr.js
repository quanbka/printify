...
let json = fs.readFileSync('fr-gelato.json');
...
for (var i = 0; i < jsonData.result.length; i++) {
    ...
    // console.log(items);
    let item = items[0];
    let url = `https://fr.api.printerval.com/order_item/${items[0].itemReferenceId}`;
    // console.log(url);
    let response = await axios.get(url)
    if (response.data.result) {
        let product_sku_id = response.data.result.product_sku_id;
        // console.log(product_sku_id);
        url = `https://printerval.com/fr/print-suppliers/${product_sku_id}/template`;
